        ...
        <div class="col-md-6">
            ...
            // if (isset($_POST['ok'])){
                ...
                $sql="SELECT DISTINCT p1.`people_id`, concat(p1.`people_name`,' ',p1.`people_surname`) as name , 
                    p2.people_stagov_id as position,
                    p3.people_dep_name as depart 
                    FROM `people` p1 
                    INNER JOIN people_pro p2 ON p1.`people_id`=p2.`people_id` 
                    INNER join people_dep p3 on p2.people_dep_id=p3.people_dep_id
                    WHERE `people_exit`=0 
                    and `people_depgroup_id`='3' 
                    and p2.people_stagov_id <> '17'
                    and p2.people_dep_id like '2%'
                    group by p1.`people_id`
                    order by p2.people_dep_id,p2.people_stagov_id
                   ";
                ...
                while ($row=mysqli_fetch_array($result)){
                    ...
                    <tr>
                        ...
                        <td class="l"><?php echo get_stagov($conn,$id); ?></td>
                        <td class="l"><?php echo $row['depart']; ?></td>
                    </tr>
                    <?php
                }
...
function get_stagov($conn,$id){
    $sql="SELECT * FROM `people_stagov` WHERE `people_stagov_id`='$id'";
    $result=mysqli_query($conn,$sql);
    $row=mysqli_fetch_array($result);
    return $row['people_stagov_name'];
}